...
struct video_frame* read_video_frame(char *buf, int *idx) {
  ...
  r = (struct video_frame*)malloc(out_size);
...
int write_video_frame(ei_x_buff* x, struct video_frame r) {
  ...
  write_frame_sound_channels(x, r.channels);
  write_frame_sound_size(x, r.size);
  write_frame_sound_rate(x, r.rate);
